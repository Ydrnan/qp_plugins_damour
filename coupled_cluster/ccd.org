...
$$ E_{CCD} = E_{HF} + \frac{1}{2} \sum_{ijab} <ij||ab>
{t_{2}}_{ij}^{ab}$$
with
...
\frac{1}{2} \sum_{kl} <kl||ij> {t_2}_{kl}^{ij} \\ &+ \sum{kc} \left(
...
#+BEGIN_SRC f90 :comments org :tangle spin_orb_ccd.irp.f
  ...
  do while (nb_iter < 10)
    ...
    call update_t2(n_occ, n_vir, r, delta,u,v, t2)
...
#+BEGIN_SRC f90 :comments org :tangle spin_orb_ccd.irp.f
subroutine update_t2(n_occ, n_vir, r, delta,u,v, t2)
