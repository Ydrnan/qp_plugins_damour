subroutine dav_dressed(dressing,Ndet,d_energies,N_st_diag,N_st)

  implicit none

  integer, intent(in)           :: Ndet, N_st_diag, N_st
  double precision, intent(in)  :: dressing(Ndet)
  double precision, intent(out) :: d_energies(N_st_diag)
  integer                       :: i,j
  logical                       :: converged
  double precision, allocatable :: d_eigenvectors(:,:), d_s2(:)

  allocate(d_eigenvectors(N_det,N_states_diag))
  allocate(d_s2(N_states_diag))

  do j = 1, N_st
    do i = 1, Ndet
      d_eigenvectors(i,j) = psi_coef(i,j)
    enddo
  enddo
  do j = N_st+1,N_st_diag
    call random_number(d_eigenvectors(:,j))
    call normalize(d_eigenvectors(:,j),Ndet)
  enddo
   
  d_s2 = 0d0

  call davidson_dressed_HS2(psi_det,               &
                         d_eigenvectors,           &
                         d_s2,                     &
                         size(d_eigenvectors,1),   &
                         d_energies,               &
                         Ndet,                     &
                         min(Ndet,N_st),           &
                         min(Ndet,N_st_diag),      &
  ...
  print*,d_energies(1:N_st)+nuclear_repulsion

  deallocate(d_eigenvectors,d_s2)

end

subroutine davidson_dressed_hs2(dets_in,u_in,s2_out,dim_in,energies,sze,N_st,N_st_diag,Nint,dressing_state,dressing,converged)
  use bitmasks
  implicit none
  BEGIN_DOC
  ...
  double precision, intent(in)   :: dressing(sze)
  ...
  if (dressing_state > 0) then
    do k=1,N_st
      do i=1,sze
        ...
        !!H_jj(i)  += u_in(i,k) * dressing_column_h(i,1)
        H_jj(i)  += u_in(i,k) * dressing(i)
      enddo
    enddo
  endif

  call davidson_dressed_diag_hjj_sjj(dets_in,u_in,H_jj,S2_out,energies,dim_in,sze,N_st,N_st_diag,Nint,dressing_state,dressing,converged)
  deallocate (H_jj)
end

subroutine davidson_dressed_diag_hjj_sjj(dets_in,u_in,H_jj,s2_out,energies,dim_in,sze,N_st,N_st_diag_in,Nint,dressing_state,dressing,converged)
  use bitmasks
  use mmap_module
  implicit none
  BEGIN_DOC
  ...
  call write_time(6)
  write(6,'(A)') ''
  write(6,'(A)') 'Davidson Diagonalization Dressed'
  write(6,'(A)') '------------------------'
  write(6,'(A)') ''
        ...
        !if (N_st == 1) then
          ...
          do istate=1,N_st_diag
            do i=1,sze
              ...
              !!W(i,shift+istate) += dressing_column_h(i,1) * U(i,shift+istate)
              W(i,shift+istate) += dressing(i) * U(i,shift+istate)
            enddo
