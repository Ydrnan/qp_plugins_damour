...
!  not_vir_converged = .TRUE.
  ...
  do l = 1, 4
    ...
    do while (not_converged .and. k <= 10000)
      ...
      print*, 'max elem in grad', tmp_list(1), tmp_list(tmp_list_size), max_elem
      ! Diagonal hessian
      call H_diag_tmp_orb_loc_v2(tmp_n, tmp_list_size, tmp_list, H)
      ! Step in the subspace
      call x_tmp_orb_loc_v2(tmp_n, tmp_list_size, tmp_list, v_grad, H, tmp_m_x)
    ...
    ! Deallocation
    deallocate(v_grad, H, tmp_m_x, tmp_R, tmp_list)
...
subroutine x_tmp_orb_loc_v2(tmp_n, tmp_list_size, tmp_list, v_grad, H, tmp_m_x)
  ...
  double precision, intent(in) :: v_grad(tmp_n)
  double precision, intent(in) :: H(tmp_n, tmp_n)
  double precision, intent(out) :: tmp_m_x(tmp_list_size, tmp_list_size)
  double precision, allocatable :: x(:)
  double precision :: lambda , accu, max_elem
  integer :: i,j,tmp_i,tmp_j,tmp_k
