...
program tdcc
  ...
  do while (not_converged)
    ...
    call compute_Q1_2(nO,nV,det,t1,t2,Q1)
    call compute_Q2_2(nO,nV,det,t1,t2,Q2)
...
subroutine compute_Q1_2(nO,nV,det,t1,t2,Q1)
  ...
  do ia = i_ia, f_ia
    ...
    do aa = i_aa, f_aa
      ...
      Q1(ia,aa) = Q1(ia,aa) & 
      -1.0d0 * t1(nb, ab) * t2(ma, ib, na, mb) & 
      -1.0d0 * t1(ma, na) * t2(nb, ib, ab, mb) & 
      -1.0d0 * t1(ib, mb) * t2(ma, nb, na, ab) & 
      +1.0d0 * t1(nb, mb) * t2(ma, ib, na, ab) & 
      -1.0d0 * t1(nb, ab) * t1(ma, na) * t1(ib, mb)
    enddo
  enddo
  ...
  do ib = i_ib, f_ib
    ...
    do ab = i_ab, f_ab
      ...
      -1.0d0 * t1(ma, aa) * t2(ia, nb, na, mb) & 
      -1.0d0 * t1(ia, na) * t2(ma, nb, aa, mb) & 
      +1.0d0 * t1(ma, na) * t2(ia, nb, aa, mb) & 
      -1.0d0 * t1(nb, mb) * t2(ma, ia, aa, na) & 
      -1.0d0 * t1(ma, aa) * t1(ia, na) * t1(nb, mb)
    enddo
  enddo
...
** Q2
#+begin_src f90 :comments org :tangle tdcc.irp.f
subroutine compute_Q2_2(nO,nV,det,t1,t2,Q2)
