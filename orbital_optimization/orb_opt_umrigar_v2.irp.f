program orb_opt_umrigar_v2
  read_wf = .True.
  TOUCH read_wf
  call run_orb_opt_umrigar_v2
end

subroutine run_orb_opt_umrigar_v2
  implicit none
  ...
  double precision, allocatable :: prev_Hm1g(:), prev2_Hm1g(:), prev_m(:), m(:)
  integer                       :: info,method
  integer                       :: n
  integer                       :: i,j,p,q,k
  ...
  allocate(prev_Hm1g(n), prev2_Hm1g(n), prev_m(n), m(n))
  ...
  do while (.not.converged)
    ...
    ! We must wait 2 iterations in order to have 3 different Hm1g vectors
    if (nb_iter >= 2) then
    ...
    ! for umrigar method's
    prev_m = m

    ! Umrigar method's
    if (nb_iter >= 2) then
      call umrigar_acc_newton_v2(n,H,prev2_Hm1g,prev_Hm1g,Hm1g,prev_m,m,f_t)
    else
      f_t = 1d0
    endif
