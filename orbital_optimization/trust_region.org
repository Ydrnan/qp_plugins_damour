...
** Code
   ...
   #+BEGIN_SRC f90 :comments org :tangle org_trust_region.irp.f
subroutine org_trust_region(n,m,method,nb_iter,thresh_eig,H,v_grad,rho,e_val,w,x,m_x,delta)
  include 'constants.h'
...
*** Initialization and norm
  ...
  ! Norm^2 of x, ||x||^2
  print*,'||x||^2 :'
  norm2_x = f_norm_trust_region_omp(n,ABS(e_val),W,v_grad,0d0,thresh_eig)
...
*** Calculation of the optimal lambda
  ...
  elseif (e_val(1) >= 0d0) then!-1d-12) then
     ...
     call org_trust_newton_omp(n,e_val,W,v_grad,delta,thresh_eig,lambda)
  ...
  norm2_x = f_norm_trust_region_omp(n,e_val,W,v_grad,lambda,thresh_eig)
  print*,''
