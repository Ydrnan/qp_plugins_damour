...
subroutine org_rotation_matrix(A,LDA,R,LDR,n,info)
  ...
  double precision              :: norm, max_elem, max_elem_A, pi
...
#+BEGIN_SRC f90 :comments org :tangle org_rotation_matrix.irp.f
  ...
  do j = 1, n
    ...
    enddo
  enddo
  print*,'rotation matrix: max element in A', max_elem_A

  if (ABS(max_elem) > 2 * pi) then
     print*,'rotation matrix: WARNING ABS(max_elem_A) > 2 pi '
  endif
...
*** Tau^-1, cos(tau), sin(tau)
  ...
  enddo
  print*,'rotation matrix: max elem taum1:', max_elem
...
*** Post conditions
  ...
  if (ABS(max_elem) > 1d-12) then
    print*, 'rotation matrix: WARNING: max error in R.R^T > 1d-12'
  endif
