...
subroutine run_extract_c
  ...
  !if (extract_c3 .or. extract_c4) then
  !  print*,''
  !  print*,'The computation of the phase factor for exc > 2 should be solved before'
  !  call abort
  !endif
                  ...
                  do tmp_j = 1, nO_S(s2)
                    ...
                    do tmp_i = 1, nO_S(s1)
                      ...
                      if (dabs(c3_bc(i,j,k,a)) > thresh_coef) then
                        ...
                        !call get_excitation(psi_det(1,1,1),tmp_det(1,1,idx_coef),exc,degree,phase,N_int)
                        call get_phase_general(psi_det(1,1,1),tmp_det(1,1,idx_coef),phase,degree,N_int)
...
subroutine overlap_wf(Ndet, list_det, list_coef)
  ...
  do s = 1, N_states
    ...
    do i = 1, size(psi_coef,1)
      ...
      enddo
    enddo
    write(*,'(A19,I2,A3,F12.6)') 'Overlap with state ', s,' = ', dabs(accu)
  enddo 
