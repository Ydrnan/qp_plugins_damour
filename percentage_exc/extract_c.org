...
subroutine run_extract_c
  ...
  integer, allocatable :: n_spin(:),list_occ(:,:), list_vir(:,:), list_spin_occ(:,:), list_spin_vir(:,:), key(:)
  ...
  call extract_list_orb_spin(psi_det(1,1,1),nO_m,nV_m,list_occ,list_vir)
  ...
  TOUCH N_states
  ! Save a one det wf
  call save_wavefunction_general(1,N_states,l_det,1,l_coef)
  ...
  ! Save the wave function
  call save_wavefunction_general(nb_coef,N_states,l_det,size(l_coef,1),l_coef)
  
  !N_states = 1
  !TOUCH N_states
  !!call save_det(nb_coef, N_states, l_det, l_coef)
  !call save_det(1, N_states, l_det, l_coef)
  !!print*,'N_det',N_det
  !!N_det = nb_coef
  !!SOFT_TOUCH psi_det psi_coef N_det !N_det_beta_unique N_det_alpha_unique psi_det_alpha_unique psi_det_beta_unique
  !!! Just to have a large enough psi_det vector...
...
subroutine save_det(new_ndet, Nstates, list_det_sorted, list_coef_sorted)
