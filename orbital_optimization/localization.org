...
!  enddo
  ...
  logical :: use_trust_region
  double precision :: ddot,  f_norm_trust_region_omp
!  !Initialization
!  not_converged = .TRUE.
!  if (n_core_orb < 2) then
...
!  not_vir_converged = .TRUE.
  ...
  do l = 1, 4
    ...
    thresh_eig = 1d-12
    nb_iter = 1 !############################### TEST ####################################
    ! pour choisir le delta et empêcher qu'il soit trop grand
    delta = 1
    ...
    do while (not_converged .and. k <= 10000)
      ...
      ! With trust region, but no step cancellation for the moment
