...
*** Initialization and norm
  ...
  ! Norm^2 of x, ||x||^2
  print*,'||x||^2 :'
  norm2_x = f_norm_trust_region_omp(n,e_val,W,v_grad,0d0,thresh_eig)
...
*** Calculation of the optimal lambda
  ...
  ! Liste des w^t.g = 0 pour éviter le 1/0 lorsqu'on active pas l'évitement
  ! des points selles
  ! Pouvoir choisir le nombre de dérivées secondes négatives voulu
  ...
  ! w_1^T . g = 0 ?
  if (ABS(wtg) < thresh_wtg) then
     norm2_x = f_norm_trust_region_omp(n,e_val,W,v_grad, - e_val(1), thresh_eig)
  ...
  elseif (dsqrt(norm2_x) <= delta .and. ABS(wtg) < thresh_wtg) then
  ...
  norm2_x = f_norm_trust_region_omp(n,e_val,W,v_grad,lambda)
  print*,''
...
*** Calculation of the step x
  ...
  ! Calculation of the step x
  do i = 1, n
     if (e_val(i) + lambda > thresh_eig) then !1d-12) then ! in order to avoid 1/0
