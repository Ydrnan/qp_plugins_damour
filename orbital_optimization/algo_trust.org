...
subroutine algo_trust_template(tmp_n, tmp_list_size, tmp_list)
  ...
  PROVIDE C_PROVIDER H_PROVIDER g_PROVIDER
  ...
  do while (not_converged)
      ...
      ! Provide the hessian and the gradient (only if nb_iter > 0,
      ! else it's already provided)
      if (nb_iter > 0) then
      ...
      do while (cancel_step)
          ...
          if (cancel_step) then
              ...
              ! Provide the new value of the criterion if it has a reason to change
              ! in this if loop (cf. orbital optimization program)
              PROVIDE C_PROVIDER
              prev_criterion = C_PROVIDER
              
              ! If the value of the criterion here has no reason to change
              ! in this if loop (cf. orbital localization program) 
              ! prev_criterion = criterion
              ! But it's already done by trust_region_rho in
              ! trust_region_is_step_cancelled
          endif      
