...
subroutine run_localization
  ...
  double precision :: criterion, angle_pre_rot, norm_grad
  integer          :: i,j,k,l,p, tmp_i, tmp_j, tmp_k
  integer          :: info
  integer          :: n, tmp_n, tmp_list_size
  double precision, allocatable :: v_grad(:), H(:,:), tmp_m_x(:,:), tmp_x(:)
...
!  print*,'HF', HF_energy
  ...
  do l = 1, 4
    ...
    do while (not_converged)
      ...
      print*, 'Max element in gradient:', max_elem
      ...
      if (nb_iter > 10000) then
        not_converged = .False.
      endif
    enddo

    ! Deallocation of temporary arrays
    deallocate(v_grad, H, tmp_m_x, tmp_R, tmp_list,tmp_x)
...
#+BEGIN_SRC  f90 :comments org :tangle localization.irp.f
subroutine gradient_tmp_orb_loc_v2(tmp_n, tmp_list_size, tmp_list, v_grad, max_elem, norm_grad)
