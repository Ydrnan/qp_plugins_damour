...
subroutine smart_xc(nO,nV,t1_cc,t2_cc)
  ...
  double precision :: thresh_t2_sf, thresh_sf
  
  integer :: sze_t2_f,sze_t11_f,sze_t12_f,sze_t22_f
  integer :: sze_t13_f,sze_t111_f
  double precision, allocatable :: t2_f(:),t11_f(:),t12_f(:),t22_f(:)
  double precision, allocatable :: t13_f(:),t111_f(:)
  integer, allocatable :: t2_idx_f(:,:),t11_idx_f(:,:),t12_idx_f(:,:),t22_idx_f(:,:)
  integer, allocatable :: t13_idx_f(:,:),t111_idx_f(:,:)

  thresh_sf = 1d-16
  thresh_t2_sf = thresh

  if (thresh_t2_sf > thresh) then
    print*,'thresh_t2_sf must be <= thresh'
    call abort
  endif

  ! We already have all the t1 > 0 
  ! Now we take all the t2 > thresh_t2_sf
  ! With that we can start to fill the square
  ! Reducing thresh_t2 will increase the computation cost...
  ! We can choose thresh_t2_sf = thresh
  !print*,'Select the t2 that will be used to fill the square...'
  call contrib_t2_nb(nO,nV,thresh_t2_sf,t2_cc,sze_t2)
  !print*,sze_t2
  deallocate(t2,t2_idx)
  allocate(t2(sze_t2),t2_idx(4,sze_t2))
  call contrib_t2(nO,nV,thresh_t2_sf,t2_cc,sze_t2,t2,t2_idx)
  ...
  ! C4
  new_sze_c4 = sze_c4 + sze_t22_f + sze_t13_f
