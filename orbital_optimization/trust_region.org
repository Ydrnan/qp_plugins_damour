...
*** Calculation of the optimal lambda
  ...
  if (.False.) then !avoid_saddle) then
    i = 1
    ! Number of negative eigenvalues
    do while (e_val(i) < 0d0)
      i = i + 1
    enddo
    n_neg_eval = i - 1
    delta_save = delta ! save
    ! Reduction of delta
    delta = delta * DBLE((n-n_neg_eval)/n)
    beta = delta_save - delta

    ! Remove the terms where e_val < 0 and w^T.g is small
    do i = 1, n_neg_eval 
...
!#  endif
  ...
  ! w_1^T . g = 0 ?
  if (DABS(tmp_wtg(1)) < thresh_wtg .and. (.not. absolute_eig)) then
  ...
  elseif (dsqrt(norm2_x) <= delta .and. DABS(tmp_wtg(1)) < thresh_wtg) then
     ...
     print*, 'Computation of the optimal lambda...'
     call org_trust_newton_omp(n,e_val,tmp_wtg,delta)
  endif
