...
subroutine org_rotation_matrix(A,LDA,R,LDR,n,info)
  ...
  double precision              :: norm, max_elem, max_elem_A
...
#+BEGIN_SRC f90 :comments org :tangle org_rotation_matrix.irp.f
  ...
  do i = 1, n
    if (A(i,i) /= 0d0) then
      print*, 'Rotation matrix: matrix is not antisymmetric'
      print*, 'Non 0 element in the diagonal', i, A(i,i)
      call ABORT
    endif
  enddo
...
#+BEGIN_SRC f90 :comments org :tangle org_rotation_matrix.irp.f
  ...
  print*,'Eigenvalue of B=A.A for rotation matrix'
  do i = 1,n
    print*, e_val(i)
  enddo
#+END_SRC
...
#+BEGIN_SRC f90 :comments org :tangle org_rotation_matrix.irp.f
  ...
  enddo
  print*,'max elem taum1:', max_elem

  max_elem = 0d0
  do i = 1, n
    if (ABS(cos_tau(i,i)) > ABS(max_elem)) then
      max_elem = cos_tau(i,i)
    endif
  enddo
  print*,'max elem cos_tau:', max_elem

  max_elem = 0d0
  do i = 1, n
    if (ABS(sin_tau(i,i)) > ABS(max_elem)) then
      max_elem = sin_tau(i,i)
    endif
  enddo
  print*,'max elem sin_tau:', max_elem
...
#+BEGIN_SRC f90 :comments org :tangle org_rotation_matrix.irp.f
  ...
  max_elem = 0d0
  do i = 1, mo_num
    if (ABS(RR_t(i,i)) > ABS(max_elem)) then
      max_elem = RR_t(i,i)
    endif
  enddo

   print*, 'Max error on the diagonal in R.R^T:', max_elem
